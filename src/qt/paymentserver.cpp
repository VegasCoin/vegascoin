...
const int VEGASCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString VEGASCOIN_IPC_PREFIX("vegascoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(VEGASCOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(VEGASCOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(VEGASCOIN_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click vegascoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start vegascoin: click-to-pay handler");
    else
...
{
    // clicking on vegascoin: URLs creates FileOpen events on the Mac:
